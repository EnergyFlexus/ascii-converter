...
int main(int argc, char** argv)
{
	...
	int rescaled_width;
	if(argc > 2) rescaled_width = std::stoi(argv[2]);
	else rescaled_width = 80;

	int rescaled_heigt;
	if(argc > 3) rescaled_heigt = std::stoi(argv[3]);
	else rescaled_heigt = (double)height * ((double)rescaled_width / (double)width) * console_rescale;
